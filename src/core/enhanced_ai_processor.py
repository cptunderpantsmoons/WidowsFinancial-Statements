...
from config.settings import (
    OPENROUTER_API_KEY, OPENROUTER_BASE_URL, TEXT_MODEL,
    API_TIMEOUT_SECONDS, ERRORS
)
...
class EnhancedAIProcessor:
    ...
    def create_enhanced_semantic_mapping(
        ...
        try:
            logger.info(f"Starting enhanced semantic mapping for {len(template_labels)} labels")
            ...
            self._update_accuracy_metrics(mapping, refined_mapping, validated_mapping, final_mapping)
            
            logger.info(f"Enhanced mapping complete with {len(final_mapping)} mappings")
            return final_mapping, confidence_scores
        
        except Exception as e:
            logger.error(f"Enhanced semantic mapping failed: {str(e)}")
            raise
    ...
    def _knowledge_based_mapping(
        ...
        for label in template_labels:
            suggestions = self.knowledge_base.suggest_mappings(label, data_accounts)
            ...
            if suggestions:
                ...
                normalized_match = self.knowledge_base.normalize_account_name(best_match)
                
                if normalized_label == normalized_match:
                    confidence = 0.95  # Very high confidence for exact semantic matches
                elif any(word in best_match.lower() for word in normalized_label.split()):
                ...
                logger.debug(f"Knowledge mapping: '{label}' -> '{best_match}' (conf: {confidence:.2f})")
    ...
    def _llm_refinement_and_validation(
        ...
        labels_for_llm = [
            label for label in template_labels
            ...
            try:
                ...
                for label in labels_for_llm:
                    if label in llm_mapping and llm_mapping[label] in data_accounts:
                        ...
                        confidence_scores[label] = (base_confidence + llm_confidence) / 2
                        
                        logger.debug(f"LLM refined: '{label}' -> '{llm_mapping[label]}' (conf: {confidence_scores[label]:.2f})")
    ...
    def _validate_formula_consistency(
        ...
        try:
            ...
            validation_results = self.knowledge_base.validate_formula_consistency(mapped_data)
            ...
            self.accuracy_metrics['validation_passed'] += sum(validation_results.values())
    ...
    def _enhance_with_benchmarks(
        ...
        for template_label, account_name in mapping.items():
            try:
                ...
                base_confidence = self.accuracy_metrics['confidence_scores'].get(template_label, 0.8)
                
                # Check benchmark context
                metric_type = self._infer_metric_type(template_label)
                benchmark_context = self.knowledge_base.get_benchmark_context(metric_type)
                ...
                if benchmark_context:
                    ...
                    logger.debug(f"Benchmark-enhanced confidence for '{template_label}': {final_confidence[template_label]:.2f}")
                else:
                    final_confidence[template_label] = base_confidence
                self.accuracy_metrics['confidence_scores'].append(final_confidence[template_label])
                
            except Exception as e:
                logger.warning(f"Benchmark enhancement failed for '{template_label}': {e}")
    ...
    def _infer_metric_type(self, label: str) -> str:
        ...
        elif any(keyword in label_lower for keyword in ['income', 'profit', 'earnings']):
            return 'metrics-generated'
        elif any(keyword in label_lower for keyword in ['cash', 'flow']):
            return 'metrics-generated'
        elif any(keyword in label_lower for keyword in ['asset', 'liability', 'equity']):
            return 'metrics-generated'
...
If no suitable match exists, use "NO_MATCH" for that label."""
        ...
        try:
            ...
            payload = {
                "model": self.model,
                "messages": [
                    {"role": "system", "content": "You are a financial accounting expert specialized in mapping financial statement labels to data accounts."},
            ...
            return {k: v for k, v in mapping.items() if v != "NO_MATCH" and v in data_accounts}
    ...
    def _update_accuracy_metrics(self, *mappings):
        ...
        if self.accuracy_metrics['confidence_scores']:
            avg_confidence = sum(self.accuracy_metrics['confidence_scores']) / len(self.accuracy_metrics['confidence_scores'])
            logger.info(f"Average mapping confidence: {avg_confidence:.3f}")
        
        knowledge_coverage = self.accuracy_metrics['knowledge_only_matches'] / max(self.accuracy_metrics['total_mappings'], 1)
    ...
    def get_quality_report(self) -> Dict:
        ...
        avg_confidence = (
            ...
            if self.accuracy_metrics['confidence_scores'] else 0
        )
        
        knowledge_coverage = (
            self.accuracy_metrics['knowledge_only_matches'] / 
            max(self.accuracy_metrics['total_mappings'], 1)
        )
        
        llm_contributions = (
            self.accuracy_metrics['llm_corrected'] / 
            max(self.accuracy_metrics['total_mappings'], 1)
        )
        
        validation_rate = (
            self.accuracy_metrics['validation_passed'] / 
            max(self.accuracy_metrics['total_mappings'], 1)
        )
        ...
        return {
            ...
            'estimated_accuracy': min(avg_confidence + (knowledge_coverage * 0.05), 0.995),
            'quality_grade': self._calculate_quality_grade(avg_confidence, knowledge_coverage, validation_rate),
            'recommendations': self._get_accuracy_recommendations(avg_confidence, knowledge_coverage)
        }
    ...
    def _calculate_quality_grade(
        ...
        overall_score = (confidence * 0.5) + (knowledge_coverage * 0.3) + (validation_rate * 0.2)
    ...
    def _get_accuracy_recommendations(self, confidence: float, knowledge_coverage: float) -> List[str]:
        """Get recommendations to improve accuracy."""
        recommendations = []
        
        if confidence < 0.90:
            recommendations.append("Consider adding more XBRL terminology to improve semantic matching")
        
        if knowledge_coverage < 0.80:
            recommendations.append("Expand knowledge base with additional industry-specific terms")
