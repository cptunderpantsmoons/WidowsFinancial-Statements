...
class FinancialValidator:
    ...
    def validate_financial_statement(
        ...
        logger.info(f"Validating {statement_type} statement with {len(data)} data points")
                ...
                for check_config in self.compliance_standards[standard]:
                    try:
                        ...
                        if not result['compliant']:
                            logger.warning(f"Non-compliance detected in {standard}: {check_config['name']}")
        ...
        try:
            ...
            for ratio_name, ratio_value in ratios.items():
                if ratio_value is not None:
                    ...
                    variance = self._calculate_variance(ratio_value, benchmark) if benchmark else None
                    interpretation = self._interpret_ratio(ratio_name, ratio_value, benchmark)
            ...
            logger.info(f"Financial analysis complete: {len(analysis_results)} metrics analyzed")
    ...
    def _execute_validation_rule(
        ...
        try:
            if '==' in formula:
                ...
                if tolerance > 0:
                    diff_pct = abs(lhs_value - rhs_value) / max(abs(lhs_value), abs(rhs_value), 1)
                ...
                return ValidationResult(
                    ...
                    message=(
                        ...
                        if not is_valid else f"Check passed: {rule['description']}"
                    ),
                ...
                return ValidationResult(
                    ...
                    message=(
                        ...
                        if not is_valid else f"Logic check passed: {rule['description']}"
                    ),
                    severity='error' if not is_valid else 'info',
                    formula_name=rule_name,
                    expected_value=rhs_value,
                ...
                # Range check
                min_val, max_val = formula.split('<=')[1].split('+')[0].strip().split()
                actual_value = self._evaluate_expression(formula.split('<=')[0].strip(), data)
                ...
                return ValidationResult(
                    ...
                    message=(
                        ...
                        if not is_valid else f"Range check passed: {rule['description']}"
                    ),
                    severity='error' if not is_valid else 'info',
                    formula_name=rule_name
                )
    ...
    def _evaluate_expression(self, expression: str, data: Dict[str, float]) -> Optional[float]:
    ...
    def _calculate_financial_ratios(self, data: Dict[str, float]) -> Dict[str, float]:
        ...
        try:
            # Current Ratio
            current_assets = self._find_value(data, ['current assets', 'total current assets'])
            current_liabilities = self._find_value(data, ['current liabilities', 'total current liabilities'])
            ...
            # Debt to Equity
            total_debt = self._find_value(data, ['total debt', 'short term debt', 'long term debt'])
            ...
            # Net Profit Margin
            net_income = self._find_value(data, ['net income', 'net profit', 'net earnings'])
    ...
    def _find_value(self, data: Dict[str, float], search_terms: List[str]) -> Optional[float]:
    ...
    def _interpret_ratio(self, ratio_name: str, value: float, benchmark: Optional[float]) -> str:
    ...
    def _check_materiality(self, data: Dict[str, float]) -> Dict:
        ...
        return {
            ...
            'interpretation': 'Material items identified and presented' if is_material else 'All amounts within normal ranges'
        }
    ...
    def _check_going_concern(self, data: Dict[str, float]) -> Dict:
        ...
        return {
            ...
            'violation': 'Negative equity may indicate going concern issues' if not is_going_concern else None,
            'recommendation': 'Address equity deficits in management discussion' if not is_going_concern else None
        }
    ...
    def get_validation_summary(self, validation_results: List[ValidationResult]) -> Dict:
        ...
        failed = total - passed
        errors = sum(1 for r in validation_results if not r.is_valid and r.severity == 'error')
        warnings = sum(1 for r in validation_results if not r.is_valid and r.severity == 'warning')
