...
class KnowledgeExtractor:
    ...
    def _load_financial_formulas(self):
        ...
        try:
            file_path = self.knowledge_path / "formulas_with_explanations_with_questions_with_gt.xlsx"
            df = pd.read_excel(file_path)
    ...
    def _load_benchmarks(self):
        ...
        try:
            ...
            logger.info(f"Loaded benchmark data for {len(self.benchmarks)} question types")
        except Exception as e:
    ...
    def _categorize_term(self, term: str) -> str:
        ...
        elif any(keyword in term.lower() for keyword in ['revenue', 'expense', 'income', 'profit']):
    ...
    def _categorize_formula(self, formula_name: str) -> str:
        ...
        if any(keyword in formula_name.lower() for keyword in ['ratio', 'margin', 'return']):
            return 'ratio'
        elif any(keyword in formula_name.lower() for keyword in ['npv', 'irr', 'pv', 'fv']):
            return 'valuation'
    ...
    def _extract_financial_patterns(self, question: str, answer: str) -> Dict:
        ...
        patterns['percentage_patterns'] = re.findall(percentage_pattern, answer, re.IGNORECASE)
        
        # Extract financial terms from question
        financial_terms = re.findall(r'\b(?:revenue|income|profit|loss|expense|asset|liability|equity|cash|debt|capital|margin|ratio)\b', question, re.IGNORECASE)
        patterns['term_associations'] = {term.lower(): answer for term in financial_terms}
    ...
    def get_synonyms(self, term: str) -> List[str]:
        ...
        for canonical_term, info in self.xbrl_terms.items():
            if canonical_term == term_normalized:
                ...
                synonyms.extend([s for s in info.get('synonyms', []) if s != term_normalized])
    ...
    def get_formulas_for_category(self, category: str) -> Dict:
        ...
        return {name: info for name, info in self.financial_formulas.items() 
    ...
    def _has_required_data(self, components: List[str], data: Dict[str, float]) -> bool:
        ...
        has_all = all(comp in ' '.join(normalized_data_keys) for comp in normalized_components)
        logger.debug(f"Components check: {normalized_components} -> {has_all}")
        return has_all
    
    def _check_formula_validity(self, formula: str, components: List[str], data: Dict[str, float]) -> bool:
    ...
    def suggest_mappings(self, template_labels: List[str], data_accounts: List[str]) -> Dict[str, List[str]]:
        ...
        for label in template_labels:
            ...
            for account in data_accounts:
                ...
                if normalized_label == normalized_account:
                    suggestions[label].append(account)
                    continue
                
                # Partial matches for longer phrases
                if any(word in normalized_account.split() for word in normalized_label.split()):
                    suggestions[label].append(account)
        ...
        for label in suggestions:
            suggestions[label] = list(set(suggestions[label]))[:3]
